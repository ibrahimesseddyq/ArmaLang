...
(function() {
    ...
    var grammar = {
        ...
        ParserRules: [{
            ...
            {
                "name": "func_def",
                "symbols": [(myLexer.has("funcdef") ? { type: "funcdef" } : funcdef), "_", (myLexer.has("identifier") ? { type: "identifier" } : identifier), (myLexer.has("lparen") ? { type: "lparen" } : lparen), "_", "func_def$ebnf$1", (myLexer.has("rparen") ? { type: "rparen" } : rparen), (myLexer.has("fatarrow") ? { type: "fatarrow" } : fatarrow), "_ml", "lambda_body"],
                "postprocess":
                    (data) => {
                        return {
                            type: "funcdef",
